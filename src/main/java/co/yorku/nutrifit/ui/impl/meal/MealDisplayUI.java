...
import co.yorku.nutrifit.ui.helper.NutrifitTableModel;
...
public class MealDisplayUI extends NutrifitWindow {
    ...
    private SimpleDateFormat simpleDateFormat = new SimpleDateFormat("MMM dd yyyy"); // Create a SimpleDateFormat object to format Date objects
    ...
    public MealDisplayUI(NutrifitWindow parent, Meal meal) {
        ...
        NutrifitTableModel foods = new NutrifitTableModel();
        ...
        this.addBasicFoodInformation(meal);
        this.updateTable(meal, foodsTable, foods, nutrientSums);

        StringBuilder sb = new StringBuilder(); // Create a String Builder
        sb.append("<html>").append("Total Nutrient Info Meal:").append("<br/>").append("<br/>");
        this.updateNutrients(nutrientSums, sb);
        sb.append("</html>");
    ...
    private void addBasicFoodInformation(Meal meal) {
    ...
    private void updateNutrients(Map<NutrientData, Double> nutrientSums, StringBuilder sb) {
    ...
    private void updateTable(Meal meal, JTable foodsTable, DefaultTableModel foods, Map<NutrientData, Double> nutrientSums) {
        ...
        for (Map.Entry<Integer, Integer> integerIntegerEntry : meal.getFoodIDAndAmounts().entrySet()) { // Loop through all the foods in this meal
            ...
            StringBuilder sb = new StringBuilder(); // Create a string builder
            sb.append("<html>");
            int total = this.updateNutrientInfo(foodNutrientInfo, nutrientSums, sb, integerIntegerEntry.getValue());
    ...
    private int updateNutrientInfo(FoodNutrientInfo foodNutrientInfo, Map<NutrientData, Double> nutrientSums, StringBuilder sb, double value) {
        int total = 0;
        for (Map.Entry<Integer, Double> integerDoubleEntry : foodNutrientInfo.getNutrientIDAndNutrientValue().entrySet()) { // Loop through every nutrient in the food
            ...
            nutrientSums.put(nutrientData, nutrientSums.getOrDefault(nutrientData, 0.0) + (integerDoubleEntry.getValue() * value)); // Increment the nutrient to the total nutrients for this meal
